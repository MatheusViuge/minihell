...
static void	redir_open(t_node *node, t_redir *redir)
{
	if (redir->type == REDIR_IN)
	{
		...
		if (node->fd_in < 0)
			return_erro("Failed to open input file", 1, NULL);
	}
	else if (redir->type == REDIR_OUT)
	{
		...
		if (node->fd_out < 0)
			return_erro("Failed to open output file", 1, NULL);
	}
	else if (redir->type == APPEND)
	{
		...
		if (node->fd_out < 0)
			return_erro("Failed to open append file", 1, NULL);
	}
}
...
static int	here_doc_handler(t_data *data, char *delimiter)
{
	...
	if (pipe(pipefd) == -1)
	{
		return_erro("Failed to create heredoc pipe", 1, data);
		return (-1);
	}
	while (1)
	{
